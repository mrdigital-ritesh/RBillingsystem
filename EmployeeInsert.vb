...
Public Class EmployeeInsert
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles Button2.Click
        ...
        Try
            ...
            Dim checkUseridQuery As String = "SELECT COUNT(*) FROM users WHERE userid = @userid AND userid != @currentUserId"
            ...
            If count > 0 Then
                MessageBox.Show("The employee ID is already in use by another employee.", "UserID Conflict", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                conn.Close()
                Return
            End If
            conn.Close()
        ...
        If ValidateFields() Then
            Try
                ...
                ' SQL Update query
                qry = "UPDATE users SET username = @username, pwd = @pwd, phone = @phone, email = @email, aadhar = @aadhar, dob = @dob, doj = @doj, photo = @photo WHERE userid = @userid"
                Dim cmd As New MySqlCommand(qry, conn)
                cmd.Parameters.AddWithValue("@userid", TextBox1.Text)
                cmd.Parameters.AddWithValue("@username", TextBox2.Text)
                cmd.Parameters.AddWithValue("@pwd", TextBox6.Text)
